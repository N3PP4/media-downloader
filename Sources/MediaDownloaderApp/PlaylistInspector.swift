...
final class PlaylistInspector {
    private let outputQueue = DispatchQueue(label: "app.media-downloader.playlist-inspector")
    ...
    func inspect(
        ...
        completion: @escaping (Result<PlaylistAvailability, Error>) -> Void
    ) throws {
        cancel()
        let currentID = UUID()
        ...
        process.terminationHandler = { [weak self] finishedProcess in
            ...
            self.outputQueue.async {
                ...
                let result: Result<PlaylistAvailability, Error>
                if finishedProcess.terminationStatus == 0 {
                    result = Result { try PlaylistAvailabilityParser.parse(self.stdoutData) }
                } else {
                    let message = String(data: self.stderrData, encoding: .utf8)
                        ?? "Playlist inspection failed"
                    result = .failure(NSError(
                        domain: "MediaDownloader.PlaylistInspector",
